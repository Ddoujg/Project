...
class GoogLeNet1d(nn.Module):  #定义网络模型
    def __init__(self, num_classes=10, aux_logits=True, init_weights=False):
        ...
        self.conv1 = BasicConv1d(3, 64, kernel_size=7, stride=2, padding=3)
    ...
    def forward(self, x):  #正向传播过程
        ...
        # N x 512 x 14 x 14
        if self.training and self.aux_logits:    # eval model lose this layer同时为训练模式并且使用分类器
            aux1 = self.aux1(x)
...
class Inception(nn.Module):  #定义inception
    ...
    def forward(self, x):  #定义inception正向传播过程
...
class InceptionAux(nn.Module):  #定义辅助分类器
    def __init__(self, in_channels, num_classes):
        ...
        self.conv = BasicConv1d(in_channels, 128, kernel_size=1)  # output[batch, 128, 4]核为1高宽不变

        self.fc1 = nn.Linear(512, 256)
        self.fc2 = nn.Linear(256, num_classes)

    def forward(self, x):  #定义分类器正向传播过程
...
class BasicConv1d(nn.Module):  #定义一个基本的卷积层模板
